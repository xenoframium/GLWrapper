...
public class StateManager {
	...
	private static Map<Integer, GlVbo> currentVBOs = new HashMap<>();
	...
	static void bindBuffer(int bufferType, GlVbo vbo) {
		if (!vbo.equals(currentVBOs.get(bufferType))) {
			currentVBOs.put(bufferType, vbo);
			glBindBuffer(bufferType, vbo.getId());
		}
	}
	
	public static GlVbo getVBO(int target) {
		return currentVBOs.get(target);
	}
